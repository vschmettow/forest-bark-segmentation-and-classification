...
# Web app for analyzing forest images: segments bark objects and classifies them using augmented model
...
def setup_classifier_model():
    """Setup YOLOv8 classifier model (Augmented)"""
    global classifier_model
    
    if classifier_model is None:
        model_path = str(PROJECT_ROOT / "Archived_2" / "data" / "models" / "yolov8_results_augmented" / "bark_classifier" / "weights" / "best.pt")
        if not os.path.exists(model_path):
            raise FileNotFoundError(f"Classifier model not found at {model_path}")
        
        print(f"Loading augmented classifier model from {model_path}...")
        classifier_model = YOLO(model_path)
        print("✓ Augmented classifier model loaded successfully!")
...
<head>
    ...
    <title>Forest Bark Analyzer - Augmented Model</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="container">
        <header>
            <h1>🌲 Forest Bark Analyzer (Augmented Model)</h1>
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description='Forest Bark Analyzer Web Interface (Augmented Model)')
    ...
    print("=" * 70)
    print("🌲 Forest Bark Analyzer Web Interface (Augmented Model)")
    print("=" * 70)
